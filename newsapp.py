...
from settings import errorpage,auth_user,auth_admin
import store
import web
...
class add_comment():
    def POST(self):
        try:
            ...
            raise web.seeother("/news/post/view/%s"%comment.postid,absolute=True)
        except Exception, e:
            return errorpage("add comment error %s"%e)    

@app.route("/post/audit1/(.*)")
class update_post():
    @auth_admin
    def GET(self,uid):
        try:
            store.Post.get(id=uid).update(status=1)
            raise web.seeother("/news/audit",absolute=True)
        except Exception, e:
            return errorpage("audit error %s"%e)    

@app.route("/post/audit2/(.*)")
class update_post():
    @auth_admin
    def GET(self,uid):
        try:
